...
import kafka.api.{OffsetCommitRequest, OffsetRequest, PartitionMetadata, PartitionOffsetRequestInfo, PartitionOffsetsResponse, TopicMetadata, TopicMetadataRequest, TopicMetadataResponse, _}
...
class KafkaCluster(val kafkaParams: Map[String, String]) extends Serializable {
  ...
  def config: ConsumerConfig = this.synchronized {
    if (_config == null) {
      _config = new ConsumerConfig(kafkaParams)
    }
    _config
  }
